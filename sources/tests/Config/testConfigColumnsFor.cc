...
#include <iomanip>
#include <Config/ColumnsFor.h>

/*!
  \file testConfigColumnsFor.cc
  \ingroup prometheus
  \ingroup tests
  \brief A collection of tests for the prometheus::config::ColumnsFor class
  \author Lars Baehren
*/
...
{
  std::cout << "\n[testColumnsFor::test_constructors]\n" << std::endl;

  int status = 0;

  std::cout << "[1] Testing ColumnsFor() ..." << std::endl;
  try {
    prometheus::config::ColumnsFor inst;
  } catch (std::exception &e) {
    std::cout << "ERROR : " << e.what() << std::endl;
    ++status;
  }

  std::cout << "[2] Testing ColumnsFor(std::string) ..." << std::endl;
  try {
    prometheus::config::ColumnsFor inst (filename);
  } catch (std::exception &e) {
    std::cout << "ERROR : " << e.what() << std::endl;
    ++status;
  }
...
{
  std::cout << "\n[testColumnsFor::test_read_config]\n" << std::endl;

  int status = 0;
  prometheus::config::ColumnsFor inst;
  std::set<std::string> columnNames;

  std::cout << "[1] Testing ColumnsFor::readConfig(std::string) ..." << std::endl;
  try {
  ...
  std::cout << "[2] Testing ColumnsFor::columnNames() ..." << std::endl;
  try {
  ...
  std::cout << "[3] Testing ColumnsFor::columnNamesFor(std::string) ..." << std::endl;
  try {
